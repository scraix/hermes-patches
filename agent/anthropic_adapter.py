...
def _merge_anthropic_default_headers(
    existing: Optional[Dict[str, str]],
    extra: Optional[Dict[str, str]],
) -> Dict[str, str]:
    """Merge Anthropic SDK client-level default headers.

    Provider-specific compatibility quirks should live in config and pass
    through this generic helper; callers may still set Anthropic beta headers
    first and let explicit custom headers override only when they deliberately
    use the same header name.
    """
    merged: Dict[str, str] = {}
    if isinstance(existing, dict):
        merged.update({str(k): str(v) for k, v in existing.items() if str(k).strip() and v is not None})
    if isinstance(extra, dict):
        merged.update({str(k): str(v) for k, v in extra.items() if str(k).strip() and v is not None})
    return merged


def _set_anthropic_default_headers(
    kwargs: Dict[str, Any],
    headers: Optional[Dict[str, str]],
) -> None:
    merged = _merge_anthropic_default_headers(kwargs.get("default_headers"), headers)
    if merged:
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    if common_betas:
        _set_anthropic_default_headers(kwargs, {"anthropic-beta": ",".join(common_betas)})
    _set_anthropic_default_headers(kwargs, default_headers)
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    if _is_kimi_coding_endpoint(base_url):
        ...
        _set_anthropic_default_headers(kwargs, {
