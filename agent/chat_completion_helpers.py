...
def try_activate_fallback(agent, reason: "FailoverReason | None" = None) -> bool:
    ...
    try:
        ...
        # Determine api_mode from the fallback route as an atomic route tuple.
        # If the fallback entry/profile explicitly declares a protocol, honor it
        # before inferring from model names. Otherwise a custom provider serving
        # GPT-5 over chat_completions can be wrongly forced to Responses API,
        # and an Anthropic-compatible custom provider can be routed through an
        # OpenAI-shaped client.
        fb_explicit_api_mode = (fb.get("api_mode") or "").strip()
        fb_api_mode = fb_explicit_api_mode or "chat_completions"
        fb_base_url = str(fb_client.base_url)
        _fb_is_azure = agent._is_azure_openai_url(fb_base_url)
        if not fb_explicit_api_mode:
