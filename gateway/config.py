    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            for plat in _shared_loop_targets:
                ...
                if plat == Platform.TELEGRAM and "history_backfill" in platform_cfg:
                    bridged["history_backfill"] = platform_cfg["history_backfill"]
                if plat == Platform.TELEGRAM and "history_backfill_limit" in platform_cfg:
                    bridged["history_backfill_limit"] = platform_cfg["history_backfill_limit"]
                if plat == Platform.TELEGRAM and "context_cache_limit" in platform_cfg:
                    bridged["context_cache_limit"] = platform_cfg["context_cache_limit"]
                if plat == Platform.TELEGRAM and "personal_workspace_chats" in platform_cfg:
                    bridged["personal_workspace_chats"] = platform_cfg["personal_workspace_chats"]
            ...
            if isinstance(telegram_cfg, dict):
                ...
                pwc = telegram_cfg.get("personal_workspace_chats")
                if pwc is not None and not os.getenv("TELEGRAM_PERSONAL_WORKSPACE_CHATS"):
                    if isinstance(pwc, (list, dict)):
                        pwc = json.dumps(pwc)
                    os.environ["TELEGRAM_PERSONAL_WORKSPACE_CHATS"] = str(pwc)
