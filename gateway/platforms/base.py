...
def _delivery_chat_id_for_event(event) -> str:
    """Return the platform delivery target for an event.

    ``SessionSource.chat_id`` is normally both the session owner and delivery
    target. Telegram personal-workspace groups deliberately split those roles:
    the session/memory owner is the authorized user's id, while replies must go
    back to the originating group stored in ``parent_chat_id``.
    """
    source = getattr(event, "source", None)
    chat_id = getattr(source, "chat_id", "")
    if (
        _platform_name(getattr(source, "platform", None)) == "telegram"
        and getattr(source, "chat_type", None) == "personal_group"
        and getattr(source, "parent_chat_id", None)
    ):
...
_MEDIA_DELIVERY_DENIED_HOME_SUBPATHS = (
    ".ssh", ".aws", ".gnupg", ".kube", ".docker", ".config", ".azure", ".gcloud", "Library/Keychains",
)
...
def _media_delivery_recency_seconds() -> float:
    ...
    if raw in ("0", "false", "no", "off", ""):
        return 0.0
    return max(0.0, _float_env(MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV, _MEDIA_DELIVERY_TRUST_RECENT_DEFAULT_SECONDS))
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    for denied in _media_delivery_denied_paths():
        try:
            ...
            continue
        if _path_is_within(resolved, denied_resolved) or resolved == denied_resolved:
            return True
    return False
...
def _log_safe_path(path: str, max_len: int = 240) -> str:
    """Return a single-line, bounded representation of a model-emitted path."""
    safe = _LOG_UNSAFE_CHARS.sub("?", str(path))
    if len(safe) > max_len:
...
class BasePlatformAdapter(ABC):
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._topic_recovery_fn: Optional[Callable[[SessionSource], Optional[str]]] = None
    ...
    def set_topic_recovery_fn(self, handler: Optional[Callable[[SessionSource], Optional[str]]]) -> None:
        """Set an optional hook for recovering canonical thread/topic ids before batching."""
    ...
    def extract_media(content: str) -> Tuple[List[Tuple[str, bool]], str]:
        ...
        media_pattern = re.compile(
            r'''[`"']?MEDIA:\s*(?P<path>`[^`\n]+`|"[^"\n]+"|'[^'\n]+'|(?:~/|/)\S+(?:[^\S\n]+\S+)*?\.(?:png|jpe?g|gif|webp|mp4|mov|avi|mkv|webm|ogg|opus|mp3|wav|m4a|flac|epub|pdf|zip|rar|7z|docx?|xlsx?|pptx?|txt|md|html?|csv|apk|ipa)(?=[\s`"',;:)\]}]|$))[`"']?'''
        )
    ...
    async def _dispatch_active_session_command(
        ...
        _delivery_chat_id = _delivery_chat_id_for_event(event)
