...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._recent_group_context: Dict[str, Deque[Message]] = {}
    ...
    def _message_thread_id_for_send(cls, thread_id: Optional[str]) -> Optional[int]:
        ...
        if normalized.startswith("group:"):
            ...
            if len(parts) < 3 or not parts[2] or parts[2] == cls._GENERAL_TOPIC_THREAD_ID:
                return None
            return int(parts[2])
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # Register handlers
            status_update = getattr(getattr(filters, "StatusUpdate", None), "ALL", None)
    ...
    def _telegram_personal_workspace_chats(self) -> set[str]:
        """Return Telegram group chats that act as private per-user workspaces.

        A personal workspace group is a private multi-window surface for the
        authorized sender, similar to opening another CLI/DM session. The chat
        must be explicitly listed here, and each message sender must pass the
        normal user authorization path before the group inherits that sender's
        private memory/session scope. Older config shapes that included
        ``user_id``/``owner_user_id`` are accepted for compatibility, but the
        owner is now resolved from the authorized sender.
        """
        raw = self.config.extra.get("personal_workspace_chats")
        if raw is None:
            raw = os.getenv("TELEGRAM_PERSONAL_WORKSPACE_CHATS", "")
        ...
        if isinstance(raw, list):
            for item in raw:
                if isinstance(item, dict):
                    ...
                    left, sep, _right = item.partition(":")
                    add(left if sep else item)
                else:
                    add(item)
            return chats
    ...
    def _is_personal_workspace_sender_authorized(self, message: Message, sender_id: str) -> bool:
        """Return whether sender already has normal DM/private access.

        Personal workspace groups are private windows for an existing user,
        not a separate group-level grant. Reuse the gateway runner's normal
        DM-shaped authorization path first so pairing-store approvals and
        existing TELEGRAM_ALLOWED_USERS / GATEWAY_ALLOWED_USERS semantics are
        honored exactly like a private chat. Group chat allowlists are
        ...
        if callable(auth_fn):
            try:
                ...
                logger.debug(
                    "[%s] Telegram personal workspace DM-shaped auth failed for user %s",
                    self.name,
                    sender_id,
                    exc_info=True,
                )

        # Unit-test / adapter-only fallback: keep this equivalent to Telegram
        # private-chat allowlist semantics, not callback-button fail-closed
        # semantics. In production the runner path above should be present and
        # carries pairing-store approval state.
        raw_allowed = self.config.extra.get("allowed_users") if self.config.extra else None
        ...
        if isinstance(raw_allowed, list):
            ...
            allowed_ids = {part.strip() for part in str(raw_allowed).split(",") if part.strip()}
        global_allowed = {part.strip() for part in os.getenv("GATEWAY_ALLOWED_USERS", "").split(",") if part.strip()}
        allowed_ids |= global_allowed
        if allowed_ids:
            return "*" in allowed_ids or sender_id in allowed_ids
        return os.getenv("TELEGRAM_ALLOW_ALL_USERS", "").lower() in {"true", "1", "yes"} or os.getenv(
            "GATEWAY_ALLOW_ALL_USERS", ""
        ).lower() in {"true", "1", "yes"}

    def _telegram_auto_personal_workspace_enabled(self) -> bool:
        raw = self.config.extra.get("auto_personal_workspace_chats")
        if raw is None:
            raw = os.getenv("TELEGRAM_AUTO_PERSONAL_WORKSPACE_CHATS", "true")
        if isinstance(raw, str):
            return raw.strip().lower() in {"true", "1", "yes", "on"}
        return bool(raw)

    async def _maybe_register_auto_personal_workspace_chat(self, message: Message) -> None:
        """Auto-promote a 1-authorized-user + bot group into a private workspace.

        This makes a newly-created Telegram group with exactly the user and
        Hermes behave like another DM/CLI window without editing config. The
        probe is deliberately fail-closed: if member count cannot be verified,
        if the sender is not authorized, or if the chat has more than two
        members, the chat is not treated as private.
        """
        if not self._telegram_auto_personal_workspace_enabled():
            return
        ...
        sender_id = str(getattr(user, "id", "")) if user else ""
        if not chat_id or chat_id in self._telegram_personal_workspace_chats():
            return
        if not hasattr(self, "_auto_personal_workspace_chats"):
            self._auto_personal_workspace_chats = set()
        if not hasattr(self, "_blocked_auto_personal_workspace_chats"):
            self._blocked_auto_personal_workspace_chats = set()
        if not hasattr(self, "_auto_personal_workspace_probe_attempted"):
            self._auto_personal_workspace_probe_attempted = set()
        if chat_id in self._auto_personal_workspace_chats or chat_id in self._blocked_auto_personal_workspace_chats:
            return
        if chat_id in self._auto_personal_workspace_probe_attempted:
            return
        self._auto_personal_workspace_probe_attempted.add(chat_id)
        if not self._is_personal_workspace_sender_authorized(message, sender_id):
            logger.info(
                "[%s] Telegram auto workspace probe skipped for chat %s: sender %s is not authorized via DM/private auth",
                self.name,
                chat_id,
                sender_id,
            )
        ...
        if bot is None:
            return
        try:
            count = await bot.get_chat_member_count(chat_id=int(chat_id))
        except TypeError:
            count = await bot.get_chat_member_count(chat_id)
        except Exception as exc:
            logger.info("[%s] Telegram auto workspace probe failed for chat %s: %s", self.name, chat_id, exc)
            return
        if int(count) == 2:
            self._auto_personal_workspace_chats.add(chat_id)
            logger.info(
                "[%s] Telegram chat %s auto-registered as personal workspace for authorized sender %s",
                self.name,
                chat_id,
                sender_id,
            )
            ...
            self._blocked_auto_personal_workspace_chats.add(chat_id)
            logger.info(
                "[%s] Telegram chat %s is not an auto personal workspace: member_count=%s",
                self.name,
                chat_id,
                count,
            )
    ...
    def _personal_workspace_user_for_message(self, message: Message) -> Optional[str]:
        if not self._is_personal_workspace_chat(message):
            return None
        user = getattr(message, "from_user", None)
        sender_id = str(getattr(user, "id", "")) if user else ""
        if self._is_personal_workspace_sender_authorized(message, sender_id):
            return sender_id
        return None

    def _telegram_history_backfill(self) -> bool:
        """Return whether group trigger messages should fetch recent chat context."""
        configured = self.config.extra.get("history_backfill")
        if configured is not None:
            if isinstance(configured, str):
                return configured.lower() in {"true", "1", "yes", "on"}
            return bool(configured)
        return os.getenv("TELEGRAM_HISTORY_BACKFILL", "false").lower() in {"true", "1", "yes", "on"}

    def _telegram_history_backfill_limit(self) -> int:
        raw = self.config.extra.get("history_backfill_limit")
        if raw is None:
            raw = os.getenv("TELEGRAM_HISTORY_BACKFILL_LIMIT", "20")
        try:
    ...
    def _telegram_context_cache_limit(self) -> int:
        raw = self.config.extra.get("context_cache_limit")
        if raw is None:
            raw = os.getenv("TELEGRAM_CONTEXT_CACHE_LIMIT", "100")
        try:
            return max(0, int(raw))
        except (TypeError, ValueError):
            return 100
    ...
    def _should_observe_unmentioned_group_message(self, message: Message) -> bool:
        ...
        if self._personal_workspace_user_for_message(message):
            return False
    ...
    def _apply_telegram_group_observe_attribution(self, event: MessageEvent) -> MessageEvent:
        ...
        if not raw_message or not self._is_group_chat(raw_message):
            return event
        if self._personal_workspace_user_for_message(raw_message):
            return event
        if self._is_personal_workspace_chat(raw_message):
            return event
    ...
    def _should_capture_channel_context(self, message: Message) -> bool:
        """Return True when this trigger should receive recent group context."""
        if not self._telegram_history_backfill():
            return False
        if not self._is_group_chat(message):
            return False
    ...
    def _telegram_context_cache_key(self, message: Message) -> str:
        chat = getattr(message, "chat", None)
        chat_id = str(getattr(chat, "id", ""))
        thread_id = getattr(message, "message_thread_id", None)
        # Privacy boundary: cache is scoped to current chat and current topic/thread.
        return f"{chat_id}:{thread_id if thread_id is not None else ''}"

    def _cache_group_context_message(self, message: Message) -> None:
        """Keep a bounded in-memory context window for visible group messages.

        This records only updates Telegram already delivered to the bot. With
        BotFather privacy mode enabled, ordinary non-command/non-mention group
        messages are never delivered, so they cannot be cached here.
        """
        if not self._is_group_chat(message):
            return
        if self._is_personal_workspace_chat(message):
            return
        limit = self._telegram_context_cache_limit()
        if limit <= 0:
            return
        if not self._format_history_message_for_context(message):
            return
        key = self._telegram_context_cache_key(message)
        cache = self._recent_group_context.get(key)
        if cache is None or cache.maxlen != limit:
            cache = deque(cache or [], maxlen=limit)
            self._recent_group_context[key] = cache
        message_id = str(getattr(message, "message_id", ""))
        if message_id and any(str(getattr(item, "message_id", "")) == message_id for item in cache):
            return
    ...
    def _fetch_cached_channel_context(self, message: Message) -> str:
        ...
        if limit <= 0:
            return ""
        cache = self._recent_group_context.get(self._telegram_context_cache_key(message))
        if not cache:
            return ""
        trigger_id = str(getattr(message, "message_id", ""))
        candidates = [item for item in cache if str(getattr(item, "message_id", "")) != trigger_id]
        if not candidates:
            return ""
        candidates = candidates[-limit:]
        lines = [self._format_history_message_for_context(item) for item in candidates]
    ...
    @staticmethod
    def _message_sort_key(message: Message) -> tuple[int, int]:
        timestamp = getattr(message, "date", None)
        ts = int(timestamp.timestamp()) if timestamp is not None else 0
        try:
    ...
    def _format_history_message_for_context(self, message: Message) -> Optional[str]:
        ...
        sender = (
            ...
            or getattr(getattr(message, "sender_chat", None), "title", None)
            or "Unknown"
        )
    ...
    def _fetch_recent_channel_context(self, message: Message) -> str:
        """Recover a minimal same-chat/thread reply chain for context.

        Telegram's bot API does not provide general-purpose backscroll like
        Discord history(). The safe universally-available primitive is the
        reply chain already embedded in the incoming update payload. We exploit
        that without claiming broader history access.
        """
        limit = self._telegram_history_backfill_limit()
        if limit <= 0:
            return ""
        ...
        trigger_chat = getattr(message, "chat", None)
        trigger_chat_id = str(getattr(trigger_chat, "id", ""))
        trigger_thread_id = getattr(message, "message_thread_id", None)
        current = getattr(message, "reply_to_message", None)

        while current is not None and len(collected) < limit:
            current_chat = getattr(current, "chat", None)
            if str(getattr(current_chat, "id", "")) != trigger_chat_id:
                break
            if getattr(current, "message_thread_id", None) != trigger_thread_id:
                break

            current_id = str(getattr(current, "message_id", ""))
        ...
        collected.sort(key=self._message_sort_key)
        lines = [self._format_history_message_for_context(item) for item in collected]
        lines = [line for line in lines if line]
        if not lines:
            return ""
    ...
    def _build_channel_context_for_trigger(self, message: Message) -> str:
        blocks = []
        cached = self._fetch_cached_channel_context(message)
        if cached:
            blocks.append(cached)
        replied = self._fetch_recent_channel_context(message)
        if replied:
            blocks.append(replied)
        return "\n\n".join(blocks)
    ...
    async def _handle_service_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """Handle Telegram service updates that can establish private workspace groups.

        These updates are not user prompts. They let a newly created "user +
        bot" group become DM-like before the first ordinary text/media message,
        so personal workspace groups do not need @mentions or wake words.
        """
    ...
    def _text_batch_key(self, event: MessageEvent) -> str:
        ...
        recovery_fn = getattr(self, "_topic_recovery_fn", None)
        if recovery_fn is not None:
            recovered_thread_id = recovery_fn(event.source)
            if recovered_thread_id:
                event.source.thread_id = str(recovered_thread_id)
    ...
    async def _handle_media_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if not update.message:
            return
        await self._maybe_register_auto_personal_workspace_chat(update.message)
    ...
    def _build_message_event(
        ...
        # Build source
        personal_workspace_user = self._personal_workspace_user_for_message(message)
        source_chat_id = str(chat.id)
        source_chat_name = chat.title or (chat.full_name if hasattr(chat, "full_name") else None)
        source_chat_type = chat_type
        source_thread_id = thread_id_str
        if personal_workspace_user:
            workspace_parts = ["group", str(chat.id)]
            if thread_id_str:
                workspace_parts.append(thread_id_str)
            source_chat_id = personal_workspace_user
            source_chat_name = source_chat_name or f"Telegram workspace {chat.id}"
            source_chat_type = "personal_group"
            source_thread_id = ":".join(workspace_parts)
        ...
        source = self.build_source(
            ...
            user_id=(
                str(user.id)
                if user
                else (source_chat_id if source_chat_type in {"dm", "personal_group", "channel"} else None)
            ),
            user_name=(
                user.full_name
                if user
                    ...
                    chat.full_name
                    if hasattr(chat, "full_name") and source_chat_type == "dm"
                    else (chat.title if source_chat_type in {"channel", "personal_group"} else None)
                )
            ),
            thread_id=source_thread_id,
            chat_topic=chat_topic,
            parent_chat_id=str(chat.id) if personal_workspace_user else None,
            message_id=str(message.message_id),
        )

        channel_context = None
        if self._should_capture_channel_context(message):
            channel_context = self._build_channel_context_for_trigger(message)
