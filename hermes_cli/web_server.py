...
def _build_models_analytics_payload(days: int = 30) -> Dict[str, Any]:
    """Build the Models dashboard analytics payload without requiring fetch()."""
    from hermes_state import SessionDB
...
async def get_models_analytics(days: int = 30):
    ...
    return _build_models_analytics_payload(days)


def _build_models_bootstrap_payload() -> Dict[str, Any]:
    """Best-effort first-screen data for Safari/iPad dashboard loads.

    iPad Safari reports failed runtime fetches as ``TypeError: Load Failed``.
    Injecting the 30d Models payload into index.html lets the page render its
    first screen even if a later network refresh fails.
    """
    payload: Dict[str, Any] = {}
    try:
        payload["analytics30"] = _build_models_analytics_payload(30)
    except Exception:
        _log.exception("building models analytics bootstrap failed")
    try:
        payload["auxiliary"] = get_auxiliary_models()
    except Exception:
        _log.exception("building auxiliary models bootstrap failed")
    try:
        payload["modelInfo"] = get_model_info()
    except Exception:
        _log.exception("building model info bootstrap failed")
    return payload
...
def mount_spa(application: FastAPI):
    ...
    def _serve_index(prefix: str = ""):
        ...
        models_bootstrap = "{}"
        try:
            models_bootstrap = json.dumps(_build_models_bootstrap_payload(), ensure_ascii=False)
        except Exception:
            _log.exception("serialising models bootstrap failed")
        if gated:
            bootstrap_script = (
                f"<script>"
                ...
                f"window.__HERMES_MODELS_BOOTSTRAP__={models_bootstrap};"
                f"</script>"
            )
        else:
            bootstrap_script = (
