...
DEFAULT_CONFIG = {
    ...
    # Skill routing guardrails. Deterministic routing is always local. The
    # semantic reranker is enabled by default but privacy-safe: it sends only a
    # sanitized task representation unless raw_opt_in is explicitly configured.
    "skill_routing": {
        "enabled": True,
        "durable_state": True,
        "semantic_reranker": {
            ...
            "base_url": "",
            "api_key": "",
            "privacy_mode": "sanitized",  # sanitized | raw_opt_in
            "timeout_seconds": 3,
            "max_candidates": 12,
            "min_confidence": 0.65,
            "allow_network": True,
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(anthropic_beta, str) and anthropic_beta.strip():
        normalized["anthropic_beta"] = [b.strip() for b in anthropic_beta.split(",") if b.strip()]
    elif isinstance(anthropic_beta, list):
        normalized["anthropic_beta"] = [
            b.strip() for b in anthropic_beta if isinstance(b, str) and b.strip()
        ]
...
_VALID_CUSTOM_PROVIDER_FIELDS = {
    ...
    "context_length", "rate_limit_delay", "extra_body", "anthropic_beta",
