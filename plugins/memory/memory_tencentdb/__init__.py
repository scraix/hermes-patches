...
MEMORY_SEARCH_SCHEMA = {
    ...
    "description": (
        ...
        "- References past conversations ('last time we talked', 'as I mentioned', 'you know I')\n"
        "- Asks about their preferences ('what do I usually...', 'my favorite...', 'how do I like...')\n"
        "- Questions imply stored knowledge ('do you remember', 'didn't I tell you', 'I thought I said')\n"
        "- Asks 'why did I...' or 'when did I...' questions about their own past actions\n"
        "- Makes statements that might contradict your current understanding\n\n"
        "Do NOT wait for explicit 'search' or 'look up' keywords. Proactively search when "
        "the context suggests the user expects you to remember something."
    ),
    "parameters": {
        "type": "object",
        "properties": {
...
CONVERSATION_SEARCH_SCHEMA = {
    ...
    "description": (
        ...
        "**Call this tool when:**\n"
        "- User asks for exact quotes or specific wording ('what exactly did I say', 'my exact words')\n"
        "- memory_tencentdb_memory_search returned no results but the query seems valid\n"
        "- User asks about a specific conversation topic that might not be in structured memories\n"
        "- You need to verify details from a past exchange\n"
        "- User says 'I told you earlier' or 'we discussed this before' and you can't find it in memories\n\n"
        "This searches raw L0 conversation logs, so it finds verbatim dialogue even if it "
        "wasn't extracted into structured memories yet."
    ),
    "parameters": {
        "type": "object",
        "properties": {
...
class MemoryTencentdbProvider(MemoryProvider):
    ...
    def shutdown(self) -> None:
        ...
        # Tear down the supervisor after session-end. GatewaySupervisor only
        # terminates a child process it started itself (its Popen handle is
        # None when we attached to an already-running external Gateway), so
        # this closes stale per-session sidecars without killing shared ones.
        supervisor = self._supervisor
        self._client = None
        self._gateway_available = False
