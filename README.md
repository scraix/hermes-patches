...
>
> **适配状态：2026-06-04 复核到官方 release `v2026.5.29.2` / Hermes Agent `v0.15.2`。本轮发现并修复了一个真实 WebUI 回归：Sessions 页面渠道筛选曾在移除陈旧 source overlay 后丢失；现在已用 targeted patch 补回 `/api/sessions?source=...`、来源计数、前端筛选按钮，并在 live Dashboard 与 clean v0.15.2 install/build 中验证。Models 页面当前 live/API/browser 验证通过，served bundle 不含旧 `Load Failed`/cache-buster 逻辑。README 只按证据分级：Verified/Partially verified/Risk，不再声称“每个细节完美可用”。本机 live runtime 是否加载补丁必须看运行版本、served asset hash、API/browser smoke 和 gateway/dashboard 重启时间，不能只凭仓库文件判断。**
