...
**🧩 Kanban 多 agent 协同能力（上游能力 + 补丁链使用边界）**
最新版 Hermes upstream 的 Kanban 已不是简单 todo list，而是 SQLite-backed 多 profile 任务板：
- CLI surface 已验证：`hermes kanban boards list`、`list`、`stats`、`assignees` 可用；`swarm`、`specify`、`decompose`、`dispatch`、`runs`、`heartbeat` 等命令存在。
- 协同模型：多 profile 原子 claim 任务，任务可有依赖，dispatcher/worker 记录 run log/heartbeat；`swarm` 可创建“并行 workers → verifier → synthesizer”的任务图。
- 补丁链安全边界：可以用 Kanban 并行做 read-only audit；但同一个 Hermes runtime/patch checkout 上只能有一个 writer，避免多个 agent 同时改 overlay、README、install.sh 或 runtime 文件。
- 验证边界：当前已验证 CLI/DB surface；未在本轮启动真实 swarm workers，因此 multi-agent execution 标为 Partially verified，而不是 fully verified。

**🧭 CLI vs gateway 加载边界**
Hermes 有多个运行进程，不能把一个面的验证冒充另一个面：
- **当前 CLI 会话**：本轮对话所在进程已加载的 Python 模块不会因为 `hermes gateway restart` 热更新。
- **新 CLI/Python 进程**：会重新 import runtime 文件，适合验证代码/overlay 新状态。
- **gateway/Telegram workers**：需要 gateway restart 才加载 Python 代码变更；restart 会打断 Telegram session，非必要不重启。
- **报告规则**：README/报告必须分开写 `current CLI`、`new process`、`gateway-loaded state`；没有 gateway post-restart smoke 时只能标 Risk/Partially verified。
...
- session_search 在群/共享上下文有防泄漏限制；本轮 live runtime focused tests 已验证工具层隔离相关回归，clean upstream installer smoke 已验证 overlay 后工具注册存在。DB FTS 查询级 `user_id` scope 和 chat/thread 级边界仍应按具体平台入口继续做 E2E 验证。
...
| 功能面 | 当前证据标签 | 本轮实测依据 |
|---|---|---|
| overlay-first installer（后端/工具） | Verified working | 在官方 release `v2026.5.29.2` / Hermes Agent `v0.15.2` clean worktree 上运行 `install.sh`；py_compile 关键文件通过；156 个聚焦回归测试通过 |
...
| Kanban CLI/DB surface | Verified working | `hermes kanban --help`、`boards list`、`list`、`stats`、`assignees` 均返回正常 |
| Kanban 多 worker swarm 执行 | Partially verified | CLI 命令和任务图能力存在，本轮未启动真实 swarm workers，故不写成 fully verified |
| Gateway loaded state | Risk / depends on restart | Python 代码变更需要 gateway restart 才加载；README 不把 clean-smoke 冒充 Telegram/gateway E2E |
| WebUI source / build / browser | Verified after fix | v0.15.2 审计发现旧 `web/src/*` 与 `hermes_cli/web_server.py` overlay 陈旧；直接移除后曾丢失 Sessions 渠道筛选。现改为 targeted patch：补回 `/api/sessions?source=...`、`sources` 计数、All/cron/cli/telegram/weixin 按钮；clean worktree `HERMES_BUILD_WEB=1` 自动 `npm ci` + `npm run build` 通过；live Dashboard Sessions 点击 `cli` 后请求 `source=cli` 且列表 1-20/1120 全为 cli；Models 侧现已加入首屏 bootstrap + fallback，iPad/Safari 即使继续请求旧 bundle 也能渲染首屏，且公网入口旧 asset URL alias 与 Nginx no-store 已验证无红字。|
| Digital-brain 99 / ReviewProposal 闭环 | Verified for current hardening slice | 2026-06-07 四端同步核验：patch repo `4ce07a1b` 已 remote readback；local overlay 与 `install.sh` 含 `HERMES_ENABLE_SYSTEM_MEMORY_GRAPH` opt-in 逻辑；installed root watchdog/target 不再 `Wants=hermes-memory-graph.service`；live duplicate root unit 为 `inactive/disabled`，user `memory-graph.service` 为 `active`，strict baseline 输出 `status=pass`、`stop_the_line=[]`、`review_queue_total=144`。`/review` WebUI 已验证候选详情显示 `Approval eligibility`、`Readback`、`Rollback`。边界：这是本轮外置大脑加固闭环，不代表 144 个 backlog 已全部人工审批，也不代表最新 upstream 全量 rebase 已完成。|
